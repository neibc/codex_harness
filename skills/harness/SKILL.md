...
5. **🚨 종료 직전 사용자에게 트리거링 안내를 반드시 출력한다.** — 본 플러그인은 `commands/`를 동봉하지 않아 마켓/심링크 어느 install 경로에서도 슬래시 명령(`/<name>`)이 노출되지 않으므로 **유일한 진입점은 사용자가 직접 입력하는 자연어 발화**다. 구성한 하네스를 다시 어떻게 부르는지(자연어 트리거 발화)만 출력하면 충분하다 — MCP/비대화형/활성화 검증 같은 부수 안내는 사용자가 필요할 때 알아볼 영역이므로 추가하지 않는다. 자세한 출력 템플릿은 본 문서 하단 "**🚨 종료 단계 — 트리거링 안내 출력**" 참조 (Phase 7 종료 후 즉시 실행, 산출물 체크리스트 검증 전).
...
> 기존 에이전트와의 재사용/중복 검토는 Phase 3-0에서 다룬다.

> **Codex 환경 안내 — 변증법/대안 검토**: Codex(GPT-5.x)는 양측 검토(devil's advocate, 방법론 비평, steelman 분석)를 자율적으로 추가하지 않는다(실측: `saju`(Claude) 변증법 키워드 7회 vs `leehongjang`(Codex) 0회). 도메인이 비판적 검토에 적합하면 사용자가 명시 요청해야 한다:
> - "변증법 phase 추가" / "양측 입장 steelman 1 섹션"
> - "방법론 비평 단계 포함" / "반대 가설 검토"
> - **적합 도메인**: 통계적 유의성 검토, 정책/윤리 평가, 디자인 다중 후보 비교, 보안 검토, 가설 검증.
> - **부적합 도메인**: 단순 데이터 카탈로그, 사실 검증, 결정론적 변환 — 변증법 phase가 어울리지 않으니 추가 요청 안 함.
...
> 0.136부터 profile은 in-file `[profiles.*]`가 아니라 별도 파일(`$CODEX_HOME/builder.config.toml`)을 base config 위에 레이어하는 v2로 이동했다. in-file `[profiles.*]` 하위호환 여부가 미확정이므로, 역할별 모델 지정은 위 profile보다 **호출 시점 `-m <model>` 직접 명시**를 우선한다.
...
> **Codex 환경 안내 — 에이전트 정의 단서**: Codex의 에이전트 정의가 빈약한 이유는 작업 원칙이 "X 한다" 단답인 경우다(실측: `leehongjang` agents/ 평균 36줄 vs `saju` 110줄, 6.9× 격차). 짧은 원칙은 모델 자율 확장 단서로 부족하다. revfactory가 권하는 "Why를 설명하라" 원칙을 Codex 환경에서 더 강하게 적용하라:
> - 작업 원칙 각 항목에 **"왜 그렇게 하는가"** 1줄 + **"안 했을 때 발생하는 문제"** 1줄을 함께 적기. 이것이 모델의 자율 판단·확장 단서가 된다.
> - 핵심 역할은 1문장이 아니라 1문단 — "무엇을 하는가"보다 "어떤 판단으로 왜 그렇게 하는가"를 담을 것.
> - **줄 수를 강제하지 마라** — 단서가 풍부하면 분량은 자연스럽게 따라온다. 줄 수 강제는 모델이 의미 없는 살을 붙이게 만든다(Goodhart 함정).
...
신규 스킬 생성 전, `프로젝트/skills/`의 기존 스킬과 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 기능이 겹치는 스킬이 다른 이름으로 누적되기 쉽다.
...
본 플러그인은 `commands/<name>.md`를 동봉하지 않으므로 **마켓플레이스 install에서도 `--dev` 심링크 경로에서도 슬래시 명령이 노출되지 않는다** — 슬래시는 플러그인이 `commands/`를 동봉할 때만 등록된다. 따라서 Codex 사용자에게는 **자연어 트리거 발화와 비대화형 호출(`codex exec "<요청>"` 또는 stdin 주입 `codex exec - "<요청>" < <skill>.md`)을 명시**하라. description은 자연어 매칭이 잘 일어나도록 적극적("pushy")으로 작성한다.
...
> exec fan-out stdout 파싱 시 `{`로 시작하지 않는 라인은 skip한다(등록된 다른 MCP 서버의 로그/에러가 stream 중간에 누출될 수 있음).
...
> 본 플러그인은 `commands/`를 동봉하지 않아 마켓/심링크 어느 경로에서도 슬래시 명령이 노출되지 않는다. 자연어 매칭 + `codex exec` (인라인 prompt) + stdin 주입 (`codex exec - "<요청>" < <skill>.md`) 모두 동작하는지 확인한다. 활성화 검증: `codex debug prompt-input "x" 2>/dev/null | grep -o '<skill-name>:[^"]*' | head -1`.
...
> **Codex 환경 안내 — 산출물 단서**: Codex(GPT-5.x)는 산출물 항목이 추상적이면 헤더만 만들고 본문을 짧게 끝낸다(`leehongjang`(Codex) vs `saju`(Claude) 사례에서 `_workspace/` 분량 29×, 보고서 단어 수 14.9× 격차 관측). revfactory 원본 체크리스트는 모든 도메인에 추상적인 게 의도이지만, Codex 환경에서는 **각 산출물의 sub-item을 도메인 맞게 구체적으로 분해하면** 모델이 본문 확장 단서를 얻는다. 예: "Phase 1 산출물"(추상) → "도메인 분석 결과: (a) 기술 스택, (b) 데이터 모델 ER, (c) 주요 모듈 목록, (d) 사용자 페르소나, (e) 핵심 워크플로우" 같이 분해. 사용자가 명시 요청 가능: "산출물 sub-item을 구체적으로 분해해줘". **분량 자체를 늘리라고는 요청하지 마라** — Goodhart의 법칙으로 모델이 의미 없는 부풀림으로 간다. 분량은 결과지 목표가 아니다.
...
**모든 워크플로우는 이 단계로 마무리한다.** 신규 구성, 부분 재실행, 운영/유지보수 변경, 어떤 경우든 사용자 응답을 끝내기 전에 반드시 아래 템플릿을 채워서 출력한다. **트리거 발화만** 안내하면 충분하다.

출력 템플릿 (이 형태 그대로, 다른 섹션 추가 금지):

```markdown
## ✅ 하네스 구성 완료 — 이제 어떻게 트리거하나요?

생성된 하네스: `<orchestrator-skill-name>`
...
- 부분 재실행/운영 변경의 경우, "달라진 발화"만 강조해 다시 안내해도 된다.
- **MCP 서버 등록, 비대화형 `codex exec`, 활성화 검증, 슬래시 명령 미노출 안내 같은 부수 정보는 출력하지 않는다.** 사용자가 필요할 때 알아볼 영역이므로 트리거 발화만 깔끔하게 전달한다. 사용자가 직접 물어볼 때만 답하라.

> 이 단계를 건너뛰면 사용자가 구성된 하네스를 영원히 못 찾을 수 있다.
...
- [ ] **🚨 종료 직전 사용자에게 트리거링 안내 출력 완료** — 자연어 트리거 발화 3개만. MCP/비대화형/활성화 검증 등 부수 안내는 포함하지 않음. 위 "🚨 종료 단계 — 트리거링 안내 출력" 섹션의 템플릿 그대로. **누락 시 작업 미완.**
