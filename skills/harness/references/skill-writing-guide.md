# 스킬 작성 가이드 (Codex port)

하네스에서 생성하는 스킬의 품질을 높이기 위한 상세 작성 가이드. SKILL.md Phase 4의 보충 레퍼런스.

> Codex 환경 안내: Codex는 plugin manifest의 `skills` 디렉토리를 자동 주입하며, 모델은 `<skills_instructions>`에 등록된 name + description으로 트리거한다. 별도 invocation 도구 없이 SKILL.md를 직접 read 한다.
...
Description은 스킬의 유일한 트리거 메커니즘이다. Codex는 `<skills_instructions>` 목록에서 name + description만 보고 모델이 SKILL.md를 read할지 결정한다.
...
> Codex `codex exec --output-schema <FILE>` 옵션으로 JSON Schema 기반 강제도 가능. 산출물이 구조화 JSON일 때 사용.
...
사용자가 매출에 대해 물으면 모델이 finance.md만 read.
...
테스트 실행에서 에이전트들의 트랜스크립트(`codex exec --json` 출력)를 관찰한다. 다음 패턴이 보이면 번들링 대상:
...
실행 시간/토큰 측정 — Codex `codex exec --json` 의 `turn.completed.usage` 이벤트에서 회수:
...
서브에이전트의 `events.jsonl`을 파싱하여 `turn.completed.usage.input_tokens + output_tokens + reasoning_output_tokens`를 합산한다. 토큰 데이터는 stream 종료 시점에 회수해야 하므로 wrapper 스크립트가 즉시 저장.
