# 스킬 테스트 & 반복 개선 가이드 (Codex port)

하네스에서 생성한 스킬의 품질을 검증하고 반복적으로 개선하는 방법론. SKILL.md Phase 6의 보충 레퍼런스.

> Codex 환경 안내: 서브에이전트 호출은 `codex exec --json --ephemeral - "<task>" < agents/<name>.md` 로 수행한다(페르소나/스킬 본문을 stdin으로 주입 — `codex exec --help`의 "instructions are read from stdin" 참조). 토큰/시간 측정은 `turn.completed.usage` 이벤트에서 회수한다.
...
각 테스트 프롬프트에 대해 두 개의 codex exec 서브프로세스를 **동시에** spawn한다:

**With-skill 실행:**
```bash
codex exec --json --ephemeral --skip-git-repo-check \
  -C _workspace/iteration-N/eval-{id}/with_skill/ \
  -s workspace-write \
  --prompt-file skills/<skill-name>/SKILL.md \
  "{테스트 프롬프트}" \
  > _workspace/iteration-N/eval-{id}/with_skill/events.jsonl
```

**Baseline 실행:**
```bash
codex exec --json --ephemeral --skip-git-repo-check \
  -C _workspace/iteration-N/eval-{id}/without_skill/ \
  -s workspace-write \
  "{테스트 프롬프트}" \
  > _workspace/iteration-N/eval-{id}/without_skill/events.jsonl
```

(Baseline은 `--prompt-file` 없이 실행. 하지만 plugin이 활성이라면 `<skills_instructions>`가 자동 주입되므로 — 진정한 baseline이 필요하면 `--ignore-user-config` 또는 별도 plugin 비활성 환경에서 실행)
...
| 기존 스킬 개선 | 수정 전 스킬 버전 (스냅샷 보존, git stash 등) |

### 3-3. 타이밍 데이터 캡처

각 실행의 `events.jsonl`에서 `turn.completed.usage`를 **즉시** 파싱하여 저장한다:
...
`total_tokens = input_tokens + output_tokens + reasoning_output_tokens`. `duration_ms`는 wrapper 스크립트가 wall-clock으로 측정 (Codex events는 ts를 직접 제공하지 않음).
...
테스트/평가 과정에서 전문 역할의 에이전트를 활용하면 품질이 향상된다. 각 역할은 `agents/<name>.md`로 정의 후 `codex exec - < <skill-or-agent>.md`로 호출.
...
**활용 시점:** "새 버전이 정말 더 나은가?"를 엄밀하게 확인하고 싶을 때.
...
2. **무게를 벌지 않는 것은 제거하라** — 트랜스크립트(`events.jsonl`)를 읽고, 스킬이 에이전트에게 비생산적인 작업을 시키고 있다면 해당 부분을 삭제한다.
...
1. 기존 스킬 목록의 description을 수집 (`codex debug prompt-input`의 `<skills_instructions>`에서 회수)
...
4. Test set 기준으로 best description 선택 (과적합 방지)
5. 최대 5회 반복

> 이 과정은 `codex exec` 자동화 스크립트로 수행한다. 토큰 비용이 높으므로 스킬이 충분히 안정화된 후 최종 단계에서 실행한다.
