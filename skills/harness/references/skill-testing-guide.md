...
> Codex 환경 안내: 서브에이전트 호출은 `codex exec --json --ephemeral --prompt-file <skill> ...`로 수행하며, 토큰/시간 측정은 `turn.completed.usage` 이벤트에서 회수한다.
...
테스트/평가 과정에서 전문 역할의 에이전트를 활용하면 품질이 향상된다. 각 역할은 `agents/<name>.md`로 정의 후 `codex exec --prompt-file`로 호출.
