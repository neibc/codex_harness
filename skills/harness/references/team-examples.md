...
```
Phase 1: codex exec --prompt-file agents/webtoon-artist.md → 패널 생성
Phase 2: codex exec --prompt-file agents/webtoon-reviewer.md → 검수
Phase 3: codex exec --prompt-file agents/webtoon-artist.md → 문제 패널 재생성 (최대 2회)
```
...
6. 모든 작업 완료 → 리더가 통합 테스트 실행 (codex exec --prompt-file agents/test-runner.md)
```
