...
5. 특히: 중간 상태에서 최종 상태로의 전환이 누락되지 않았는지
```
...
### 3-1. read-only sandbox가 아닌 workspace-write sandbox를 사용하라

QA 에이전트가 `--sandbox read-only`이면 읽기만 가능하다. 하지만 효과적인 QA는:
...
**권장 패턴**: 각 백엔드 API 완성 시 즉시 해당 API + 대응 훅의 교차 검증 수행 (incremental QA). 팀 모드에서는 QA 에이전트가 `recv_messages`로 "API X 완성" 알림을 받아 즉시 검증 후 `send_message`로 결과 전달.
...
- 매 turn 시작 시 recv_messages({team_id, as: "qa-inspector"}) 호출
- 발견 즉시 해당 에이전트에게 send_message({to: "<implementer>", content: "<file>:<line> + 수정 방법"})
- 경계면 이슈는 양쪽 에이전트 **모두**에게 send_message
- 리더에게: task_update({status: "completed", metadata: {output: <report-path>, summary: ...}})
```
...
이 가이드의 모든 내용은 실제 버그에서 추출한 교훈이다:
