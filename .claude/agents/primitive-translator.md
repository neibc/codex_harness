...
description: Claude Code의 primitive(Skills, Agents, Teams, Tools)를 Codex CLI의 primitive(Prompts, MCP, AGENTS.md, exec subprocess)로 매핑하는 번역 테이블과 Agent Team 에뮬레이션 전략을 설계한다.
model: opus
...
- 스킬 참조: `claude-codex-translation`, `team-emulation-mcp`
...
2. **Agent Team 에뮬레이션 — 채택 안**:
   ...
   - 거절된 후보 2개 + 거절 사유
3. **Skill 시스템 매핑** — Claude SKILL.md → Codex prompt 파일 변환 규칙. frontmatter 어떻게 옮길지.
4. **에이전트 정의 매핑** — `.claude/agents/*.md` → Codex 측 표현 (별도 prompt 파일, AGENTS.md 섹션, MCP 도구 래퍼 중 어느 것?)
5. **CLAUDE.md → AGENTS.md 매핑** — 무엇을 옮기고 무엇을 버릴지
6. **빌드/배포 매핑** — `.claude-plugin/plugin.json` ↔ Codex 플러그인 매니페스트
7. **변환 불가 항목** — 명시적 기능 손실 리스트와 사용자 안내문 초안

## 협업 — 팀 모드

이 단계는 **에이전트 팀 모드**로 동작한다. `claude-harness-cartographer`와 `codex-internals-analyst`의 산출물을 읽고, 모호한 항목은 `SendMessage`로 해당 분석 에이전트에게 재질의한다(팀이 같은 phase에 살아있을 때만). 산출물은 `_workspace/03_translation_table.md` 파일로 떨어뜨린다.

`codex-plugin-builder`가 이 테이블을 빌드 명세로 사용한다. 빌더가 구현 중 모호점을 발견하면 `SendMessage`로 다시 질문할 수 있다.
