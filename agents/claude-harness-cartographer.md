...
> - 권장 sandbox: `read-only` (인벤토리 작성 시 _workspace/ 쓰기는 제외 — `--add-dir _workspace/` 또는 `workspace-write`로 보강).
> - 도구: Read / Grep / Glob / shell. 외부 네트워크 불필요.

## 핵심 역할

호출자가 지정한 플러그인 디렉토리(예: `~/.claude/plugins/cache/harness-marketplace/harness/<version>/` 또는 사용자 지정 경로)를 전수 조사하여:
...
- 사용자가 지정한 플러그인 경로 (기본 후보: `~/.claude/plugins/cache/harness-marketplace/harness/<version>/`)
- 오케스트레이터 지시
...
codex exec --json --ephemeral --skip-git-repo-check \
  -C _workspace/cartographer/ --add-dir _workspace/ \
  -s read-only \
  -o _workspace/cartographer/last.txt \
  --prompt-file agents/claude-harness-cartographer.md \
  "revfactory/harness 1.2.0 인벤토리를 _workspace/02_claude_primitives.md에 작성"
```
